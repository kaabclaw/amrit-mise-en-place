...
CREATE TABLE IF NOT EXISTS public.sops (
    ...
    tags            TEXT[] DEFAULT '{}',
    important_rules JSONB DEFAULT '[]'::jsonb,
...
CREATE OR REPLACE FUNCTION public.sops_update_search_vector()
RETURNS TRIGGER AS $$
BEGIN
    NEW.search_vector :=
        setweight(to_tsvector('english', coalesce(NEW.title, '')), 'A') ||
        setweight(to_tsvector('english', coalesce(NEW.outlet, '')), 'B') ||
        setweight(to_tsvector('english', coalesce(NEW.department, '')), 'B') ||
        setweight(to_tsvector('english', coalesce(array_to_string(NEW.tags, ' '), '')), 'C') ||
        setweight(to_tsvector('english', coalesce(NEW.important_rules::text, '')), 'D') ||
        setweight(to_tsvector('english', coalesce(NEW.steps::text, '')), 'D') ||
        setweight(to_tsvector('english', coalesce(NEW.sections::text, '')), 'D');
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
DROP TRIGGER IF EXISTS sops_search_vector_update ON public.sops;
CREATE TRIGGER sops_search_vector_update
    BEFORE INSERT OR UPDATE ON public.sops
    FOR EACH ROW EXECUTE FUNCTION public.sops_update_search_vector();
...
SELECT
    id, slug, title, type, department, outlet, tags, version,
